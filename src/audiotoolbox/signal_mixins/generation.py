...
class GenerationMixin:
    ...
    def add_noise(
        ...
        """
        # Only reseed when a seed is explicitly given. Seeding on ``None``
        # would pull fresh entropy on every call, which makes it impossible to
        # draw several independent tokens from one seeded stream (see
        # add_uncorr_noise).
        if seed is not None:
            np.random.seed(seed)
    ...
    def add_uncorr_noise(
        ...
        ----------
        corr : float, optional
            Desired correlation of the noise tokens, (default=0). A negative
            value is supported for two channels (realised by sign-inverting
            one channel); for more than two channels the positive magnitude is
        ...
        # A negative correlation is realised for two channels by sign-inverting
        # one of them (see below). For more than two channels a uniform
        # negative correlation is not achievable, so fall back to the positive
        # magnitude and warn.
        if negative and n_channels != 2:
            warnings.warn(
                "Negative correlations are only supported for two channels; "
                "using the positive magnitude instead. The resulting "
        ...
        # Seed once here so the N+1 noise tokens are drawn from a single
        # advancing RNG stream and are therefore independent. Passing the seed
        # to add_noise on every iteration would reseed the generator
        # identically each time, producing identical (degenerate) tokens.
        if seed is not None:
            np.random.seed(seed)
        noise = audio.Signal(n_channels + 1, self.duration, self.fs)
        for ch in range(noise.n_channels):
            noise.ch[ch].add_noise(ntype=ntype, seed=None)
        noise -= noise.mean(axis=0)
        ...
        # Realise a negative correlation by inverting one of the two channels.
        # This flips the sign of the cross-correlation while leaving the unit
        # variance untouched.
        if negative:
            res_noise[:, 1] *= -1
